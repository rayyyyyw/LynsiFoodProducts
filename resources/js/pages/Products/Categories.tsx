    ...
    return (
        <div className="relative">
            <select
                ...
                value={value}
                onChange={onChange}
                className="h-10 w-full cursor-pointer appearance-none rounded-lg border border-border bg-background px-3 pr-9 text-sm font-medium text-foreground shadow-sm transition-colors hover:border-primary/40 focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none"
            >
...
export default function Categories({
    ...
    const [selectedCategoryIdForFlavors, setSelectedCategoryIdForFlavors] =
        useState<number | ''>('');
    const [selectedCategoryIdForPriceList, setSelectedCategoryIdForPriceList] =
        useState<number | ''>('');
    ...
    const openEdit = (cat: Category) => {
        ...
        editForm.setData({
            ...
            slug: cat.slug,
            flavors: cat.flavors ?? [],
            price_list: (cat.price_list ?? []).map((p) => ({
                size: p.size ?? '',
    ...
    const handleSelectCategoryForFlavors = (
    ...
    const handleSelectCategoryForPriceList = (
        e: React.ChangeEvent<HTMLSelectElement>,
    ) => {
        const id = e.target.value ? Number(e.target.value) : '';
    ...
    const addFlavor = (key: 'flavorsForm' | 'editForm') => {
        if (key === 'flavorsForm')
            flavorsForm.setData('flavors', [...flavorsForm.data.flavors, '']);
    ...
    const removeFlavor = (index: number, key: 'flavorsForm' | 'editForm') => {
        if (key === 'flavorsForm')
            flavorsForm.setData(
                'flavors',
                flavorsForm.data.flavors.filter((_, i) => i !== index),
            );
        else
            editForm.setData(
                'flavors',
                editForm.data.flavors.filter((_, i) => i !== index),
            );
    };
    const setFlavor = (
        index: number,
        value: string,
        key: 'flavorsForm' | 'editForm',
    ) => {
        if (key === 'flavorsForm') {
            const f = [...flavorsForm.data.flavors];
            f[index] = value;
            flavorsForm.setData('flavors', f);
        } else {
            const f = [...editForm.data.flavors];
            f[index] = value;
            editForm.setData('flavors', f);
        }
    };

    const addPrice = (key: 'priceListForm' | 'editForm') => {
        if (key === 'priceListForm')
            priceListForm.setData('price_list', [
                ...priceListForm.data.price_list,
                { size: '', price: '' },
            ]);
        else
            editForm.setData('price_list', [
                ...editForm.data.price_list,
                { size: '', price: '' },
            ]);
    };
    const removePrice = (index: number, key: 'priceListForm' | 'editForm') => {
        if (key === 'priceListForm')
            priceListForm.setData(
                'price_list',
                priceListForm.data.price_list.filter((_, i) => i !== index),
            );
        else
            editForm.setData(
                'price_list',
                editForm.data.price_list.filter((_, i) => i !== index),
            );
    };
    const setPrice = (
        index: number,
        field: 'size' | 'price',
        value: string,
        key: 'priceListForm' | 'editForm',
    ) => {
        if (key === 'priceListForm') {
            const p = [...priceListForm.data.price_list];
            p[index] = { ...p[index], [field]: value };
            priceListForm.setData('price_list', p);
        } else {
            const p = [...editForm.data.price_list];
            p[index] = { ...p[index], [field]: value };
            editForm.setData('price_list', p);
        }
    };
    ...
    const handleSaveFlavors = (e: React.FormEvent) => {
        ...
        const flavors = flavorsForm.data.flavors.filter(
            (f) => String(f ?? '').trim() !== '',
        );
        ...
        flavorsForm.put(
            `/products/categories/${selectedCategoryIdForFlavors}`,
            {
                preserveScroll: true,
    ...
    const handleSavePriceList = (e: React.FormEvent) => {
        ...
        priceListForm.put(
            `/products/categories/${selectedCategoryIdForPriceList}`,
            {
                preserveScroll: true,
    ...
    const handleEdit = (e: React.FormEvent) => {
        ...
        const flavors = editForm.data.flavors.filter(
            (f) => String(f ?? '').trim() !== '',
        );
        editForm.transform(() => ({
    ...
    const handleDelete = (cat: Category) => {
        if (
            !confirm(
                `Delete category "${cat.name}"? Products in this category will also be deleted.`,
            )
        )
            return;
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Add a food category, then set its flavors and
                        sizes/prices. Products use these when you add them.
                    </p>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                These appear when adding a product. Add new
                                categories below.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                                ...
                                {categories.length === 0 ? (
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {categories.map((cat) => (
                                            <span
                                                key={cat.id}
                                                className="inline-flex items-center rounded-full border bg-background px-2.5 py-1 text-xs font-medium text-foreground shadow-sm"
                                            >
                                                {cat.name}
                                            </span>
                                        ))}
                            ...
                            <form onSubmit={handleAdd} className="space-y-3">
                                ...
                                <Input
                                    ...
                                    onChange={(e) =>
                                        addForm.setData('slug', e.target.value)
                                    }
                                    className="text-sm"
                                />
                                <Button
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Pick a food category, then manage its flavors.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-1.5">
                                <Label
                                    htmlFor="flavors-category"
                                    className="text-xs"
                                >
                                    Category
                                </Label>
                            ...
                            {selectedCategoryForFlavors ? (
                                <form
                                    onSubmit={handleSaveFlavors}
                                    className="space-y-4"
                                >
                                    {/* Flavor chips */}
                                    <div className="min-h-12 rounded-lg border bg-muted/10 p-2.5">
                                        {flavorsForm.data.flavors.filter((f) =>
                                            String(f ?? '').trim(),
                                        ).length === 0 ? (
                                            <p className="py-1 text-center text-sm text-muted-foreground">
                                                No flavors yet. Add one below.
                                            </p>
                                        ) : (
                                            <div className="flex flex-wrap gap-2">
                                                {flavorsForm.data.flavors.map(
                                                    (f, i) => {
                                                        if (
                                                            !String(
                                                                f ?? '',
                                                        ...
                                                        return (
                                                            <span
                                                                key={i}
                                                                className="group inline-flex items-center gap-1 rounded-full border bg-primary/5 px-3 py-1.5 text-xs font-medium text-foreground shadow-sm transition-colors hover:bg-primary/10"
                                                            >
                                                                {f}
                                                                <button
                                                                    type="button"
                                    ...
                                    <Button
                                        type="submit"
                                        className="w-full"
                                        disabled={flavorsForm.processing}
                                    >
                                        {flavorsForm.processing
                                            ? 'Saving...'
                                            : 'Save flavors'}
                                    </Button>
                                ...
                                <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed bg-muted/5 py-8 text-center">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Select a category above to manage its
                                        flavors.
                                    </p>
                                </div>
                            )}
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Pick a food category, then manage sizes and
                                prices.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-1.5">
                                ...
                                <StyledSelect
                                    id="price-category"
                                    value={String(
                                        selectedCategoryIdForPriceList,
                                    )}
                            ...
                            {selectedCategoryForPriceList ? (
                                ...
                                >
                                    <div className="space-y-2">
                                        {priceListForm.data.price_list
                                            ...
                                            <div className="space-y-2">
                                                {priceListForm.data.price_list.map(
                                                    (p, i) => (
                                                        <div
                                                            key={i}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        <Button
                                            type="submit"
                                            size="sm"
                                            disabled={priceListForm.processing}
                                        >
                                            {priceListForm.processing
                                                ? 'Saving...'
                                                : 'Save prices'}
                                        </Button>
                                    </div>
                                ...
                                <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed bg-muted/5 py-8 text-center">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Select a category above to manage sizes
                                        and prices.
                                    </p>
                                </div>
                            )}
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Overview of all categories. Click edit to change
                            everything at once.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {categories.length === 0 ? (
                            ...
                            <ul className="divide-y rounded-lg border">
                                {categories.map((cat) => {
                                    const flavorCount = (cat.flavors ?? [])
                                        .length;
                                    const sizeCount = (cat.price_list ?? [])
                                        .length;
                                    return (
                                        <li
                                            key={cat.id}
                                            className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 px-4 py-3 first:rounded-t-lg last:rounded-b-lg hover:bg-muted/30 sm:flex-nowrap"
                                        >
                ...
                >
                    <DialogContent className="max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Update name, slug, flavors, and price list.
                            </p>
                        </DialogHeader>
                        <form onSubmit={handleEdit} className="space-y-5">
                            ...
                            <div className="space-y-2">
                                ...
                                {editForm.data.price_list.map((p, i) => (
                                    <div
                                        key={i}
                                        className="flex flex-wrap items-center gap-2 rounded-lg border bg-muted/5 p-2"
                                    >
                                        <div className="flex items-center">
                                            <Input
                                                placeholder="Size"
