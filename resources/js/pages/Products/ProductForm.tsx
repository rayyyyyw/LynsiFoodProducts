...
type Variant = { id?: number; size: string; flavor: string; price: string; sku: string; stock_quantity: number };
type Product = {
    id: number;
    name: string;
    slug: string;
...
function getPriceForSize(priceList: PriceListItem[] | undefined, size: string): string {
    if (!priceList?.length || !size) return '';
    const item = priceList.find((p) => String(p.size).trim() === String(size).trim());
...
export default function ProductForm({
    ...
    const updateVariant = (index: number, field: keyof Variant, value: string | number) => {
        setVariants((v) => {
            const next = v.map((x, i) => (i === index ? { ...x, [field]: value } : x));
            if (field === 'size' && selectedCategory?.price_list?.length) {
                const sizeStr = String(value);
                const price = getPriceForSize(selectedCategory.price_list, sizeStr);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const url = isEdit ? `/products/products/${product.id}` : '/products/products';
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Name, category, description, expiry, and image.</CardDescription>
                        </CardHeader>
                            ...
                            <div className="space-y-2">
                                ...
                                {product?.image_url && !imageFile && (
                                    <div className="mb-2">
                                        ...
                                        />
                                        <p className="mt-1 text-xs text-muted-foreground">Current image. Upload a new file to replace.</p>
                                    </div>
                                )}
                                <Input
                                    type="file"
                    ...
                    <Card>
                        <CardHeader className="space-y-1">
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <CardTitle>Sizes, flavors & prices</CardTitle>
                                    <CardDescription className="mt-1">
                                        Add at least one variant (e.g. BBQ 100g = ₱50, Original 50g = ₱30).
                                    </CardDescription>
                                </div>
                                <Button type="button" variant="outline" size="sm" className="shrink-0" onClick={addVariant}>
                                    <Plus className="mr-2 size-4" />
                                    Add variant
                                </Button>
                            </div>
                            {selectedCategory && (sizeOptions.length > 0 || flavorOptions.length > 0) && (
                                <div className="rounded-md border border-border/80 bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                                    Size and flavor options come from the selected category. Price can be adjusted.
                                </div>
                            )}
                        </CardHeader>
                        <CardContent className="space-y-4">
                                            ...
                                            <div className="grid flex-1 grid-cols-2 gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)_90px_80px_auto] sm:gap-5">
                                                    ...
                                                    {sizeOptions.length > 0 ? (
                                                        ...
                                                        >
                                                            ...
                                                            <SelectContent className="rounded-lg border-border/80 shadow-lg">
                                                                {sizeOptions.map((pl) => (
                                                                    <SelectItem key={pl.size} value={pl.size} className="rounded-md py-2">
                                                                        {displaySizeWithUnit(pl.size)}
                                                                    </SelectItem>
                                                                ))}
                                                                {v.size && !sizeOptions.some((pl) => pl.size === v.size) && (
                                                                    <SelectItem value={v.size} className="rounded-md py-2">
                                                                        {displaySizeWithUnit(v.size)}
                                                                    </SelectItem>
                                                                )}
                                                            </SelectContent>
                                                        </Select>
                                                ...
                                                <div className="space-y-1.5">
                                                    ...
                                                    {flavorOptions.length > 0 ? (
                                                        <Select
                                                            value={v.flavor || undefined}
                                                            onValueChange={(val) => updateVariant(i, 'flavor', val ?? '')}
                                                        >
                                                            <SelectTrigger className="h-10 rounded-lg">
                                                            ...
                                                            <SelectContent className="rounded-lg border-border/80 shadow-lg">
                                                                {flavorOptions.map((f) => (
                                                                    <SelectItem key={f} value={f} className="rounded-md py-2">
                                                                        {f}
                                                                    </SelectItem>
                                                                ))}
                                                                {v.flavor && !flavorOptions.includes(v.flavor) && (
                                                                    <SelectItem value={v.flavor} className="rounded-md py-2">
                                                                        {v.flavor}
                                                                    </SelectItem>
                                                                )}
                                                            </SelectContent>
                            ...
                            {errors.variants && <InputError message={errors.variants} />}
                        </CardContent>
                    </Card>
