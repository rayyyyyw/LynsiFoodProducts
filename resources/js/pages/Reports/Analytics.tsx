...
type AnalyticsData = {
    ...
    top_customers: { name: string; email: string | null; orders_count: number; spent: number }[];
};
...
export default function AnalyticsPage() {
    ...
    const data: AnalyticsData = analytics ?? {
        range: 'days',
        summary: { orders: 0, revenue: 0, avg_order_value: 0, unique_customers: 0 },
        trend: [],
        top_products: [],
        top_customers: [],
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-4 md:p-6">
                <div className="flex flex-wrap items-end justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                            Business performance overview with real sales, customer, and product data.
                        </p>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground">Completed or processing orders</p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{peso(data.summary.revenue)}</div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{peso(data.summary.avg_order_value)}</div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{data.summary.unique_customers}</div>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    <Card className="lg:col-span-2">
                        <CardHeader>
                            ...
                            <CardDescription>Period-by-period order count and revenue.</CardDescription>
                        </CardHeader>
                        <CardContent>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {data.trend.map((row, i) => (
                                            <tr key={`${row.label}-${i}`} className="border-b last:border-0 hover:bg-muted/20">
                                                <td className="px-4 py-2.5 text-muted-foreground">{row.label}</td>
                                                <td className="px-4 py-2.5 font-medium">{row.orders}</td>
                                                <td className="px-4 py-2.5 text-right font-semibold">{peso(row.revenue)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Highest product revenue in selected range.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-2">
                            {data.top_products.length === 0 ? (
                                ...
                                data.top_products.map((p, idx) => (
                                    <div key={`${p.name}-${idx}`} className="rounded-lg border p-2.5">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Customers with highest spending in selected range.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="overflow-hidden rounded-lg border">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {data.top_customers.length === 0 ? (
                                        ...
                                        data.top_customers.map((c, idx) => (
                                            <tr key={`${c.name}-${idx}`} className="border-b last:border-0 hover:bg-muted/20">
                                                <td className="px-4 py-2.5">
                                                    <p className="font-medium">{c.name}</p>
                                                    {c.email && <p className="text-xs text-muted-foreground">{c.email}</p>}
                                                </td>
                                                <td className="px-4 py-2.5">{c.orders_count}</td>
                                                <td className="px-4 py-2.5 text-right font-semibold">{peso(c.spent)}</td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
