...
function Avatar({ user, size = 'sm' }: { user: UserRow; size?: 'sm' | 'lg' }) {
    ...
    return (
        <div
            className={cn(
                dim,
                'flex shrink-0 items-center justify-center rounded-full font-semibold ring-2 ring-border',
                textSize,
                color,
            )}
        >
...
function formatAddress(user: UserRow): string | null {
    const parts = [user.address, user.city, user.province, user.zip].filter(
        Boolean,
    );
...
export default function Customers() {
    ...
    const handleToggleActive = (user: UserRow) => {
        ...
        if (!confirm(`Are you sure you want to ${action} "${user.name}"?`))
            return;
        router.patch(
            `/dashboard/customers/${user.id}/toggle-active`,
            {},
            { preserveScroll: true },
        );
    };
    ...
    const handleDelete = (user: UserRow) => {
        if (
            ...
            return;
        router.delete(`/dashboard/customers/${user.id}`, {
            preserveScroll: true,
        });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-4 md:p-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                            View and manage the customers who shop at Lynsi Food
                            Products.
                        </p>
                    </div>
                    ...
                    {statCards.map((stat) => (
                        <Card key={stat.key}>
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                ...
                                <stat.icon
                                    className={cn('size-4', stat.className)}
                                />
                            </CardHeader>
                            <CardContent>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Basic information about your registered users. Use
                            the actions menu to view, deactivate, or delete a
                            user.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                ...
                                <tbody>
                                    {list.length === 0 ? (
                                        <tr>
                                            ...
                                            >
                                                No customers found yet. New user
